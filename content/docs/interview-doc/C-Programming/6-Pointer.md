...
1. 数组名在其作用域内编译器认为它是数组；
2. 在作用域外数组名退化为指向数组第 0 个元素的指针；
...
2. 形如`a`,`*a`,`a[0]`,`*(a+0)`,`&a[0]`,`&a[0][0]`的均为首地址；
...
括号中的`*`表明 p 是指针，它指向一个数组，数组的类型为`int [4]`，这正是 a 所包含的每个一维数组的类型。因此称p是二维数组指针，它和a的使用方式一模一样，通过它也能够访问二维数组a中的全部元素。

**特别地，数组名 a 在表达式中会被转换为和 p 等价的指针！**

注意：若写作`int *p[4]`，那么应该理解为`int *(p[4])`，p 就成了一个指针数组，而不是二维数组指针。
